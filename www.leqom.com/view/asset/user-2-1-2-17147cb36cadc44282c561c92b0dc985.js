...
----------------------------------- */
$(window).on('load', function(){
    $("#splash_bg").slideUp(1000);
    $('#fv_bg').stop().animate({ opacity: 1},1500 ,function(){
        $('.m_fixed_wavy_lines').slideDown(500, function(){
            $('#fv_wrap .item_img').stop().animate({ opacity: 1},500, function(){
                $('#fv_wrap .item_name').stop().animate({ opacity: 1}, 1000);

                $('#fv_wrap').find('.js_fv_fade_up_one').each(function(index){
...
$(window).on('load scroll', function(){
    ...
    $('.js_fade_up_one_wrap.intro_fade').find('.js_fade_up_one').each(function(index){
        const elemPos = $(this).offset().top;
        if (fadeUpScroll > elemPos - fadeUpWindowHeight + fadeUpTiming){
    ...
    $('.js_fade_up_one_wrap.live_fade').find('.js_fade_up_one').each(function(index){
        const elemPos = $(this).offset().top;
        if (fadeUpScroll > elemPos - fadeUpWindowHeight + fadeUpTiming){
            $(this).delay(index * 70).queue(function(){
                $(this).addClass('is_fade_up');
    ...
    $('.js_fade_up_one_wrap.our_fade').find('.js_fade_up_one').each(function(index){
        const elemPos = $(this).offset().top;
        if (fadeUpScroll > elemPos - fadeUpWindowHeight + fadeUpTiming){
            $(this).delay(index * 70).queue(function(){
                $(this).addClass('is_fade_up');
    ...
    $('.js_fade_up_one_wrap.info_fade').find('.js_fade_up_one').each(function(index){
        const elemPos = $(this).offset().top;
        if (fadeUpScroll > elemPos - fadeUpWindowHeight + fadeUpTiming){
            $(this).delay(index * 70).queue(function(){
                $(this).addClass('is_fade_up');
...
$(window).on('load scroll', function(){
    ...
    $('#fv_bg').css({
        transform: 'scale('+(100 + scroll / 50) / 100 +')',//scroll / 50の値を小さくすると拡大値が大きくなります。
        top: -(scroll/50) + "%",
    });
});
